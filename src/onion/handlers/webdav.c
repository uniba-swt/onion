...
onion_connection_status onion_webdav_get(const char *filename,
                                         onion_webdav * wd, onion_request * req,
                                         onion_response * res);
...
onion_connection_status onion_webdav_handler(onion_webdav * wd,
  ...
  char filename[512];
  snprintf(filename, sizeof(filename), "%s/%s", wd->path,
...
onion_connection_status onion_webdav_delete(const char *filename,
  ...
  if (error == 0)
    ...
    return onion_shortcut_response("Could not delete resource",
                                   HTTP_INTERNAL_ERROR, req, res);
  }
}
...
onion_connection_status onion_webdav_move(const char *filename,
  ...
  if (strncmp(fullpath, dest, fpl - ppl) != 0) {
    ...
    tmp[l] = 0;
    ONION_WARNING("Move to out of this webdav share! (%s is out of %s)", dest,
                  tmp);
    return onion_shortcut_response("Moving out of shared share", HTTP_FORBIDDEN,
                                   req, res);
  }
  dest = &dest[fpl - ppl];
  ...
  if (ok == 0) {
    ...
    return onion_shortcut_response("Could not create resource", HTTP_FORBIDDEN,
                                   req, res);
  }
}
...
onion_connection_status onion_webdav_put(const char *filename,
  ...
  if (ok == 0) {
    ...
    ONION_ERROR("Could not rename %s to %s (%s)", tmpfile, filename,
                strerror(errno));
    return onion_shortcut_response("Could not create resource", HTTP_FORBIDDEN,
                                   req, res);
  }
...
static int onion_webdav_parse_propfind(const onion_block * block) {
  ...
  xmlDocPtr doc;
  doc =
      xmlParseMemory((char *)onion_block_data(block), onion_block_size(block));
...
int onion_webdav_write_props(xmlTextWriterPtr writer, const char *basepath,
                             const char *realpath, const char *urlpath,
                             const char *filename, int props) {
  ONION_DEBUG("Info for path '%s', urlpath '%s', file '%s', basepath '%s'",
              realpath, urlpath, filename, basepath);
  ...
  xmlTextWriterWriteAttribute(writer, BAD_CAST "xmlns:a",
                              BAD_CAST "http://apache.org/dav/props/");
  ...
  if (props & WD_LAST_MODIFIED) {
    ...
    xmlTextWriterWriteElement(writer, BAD_CAST "lp1:getlastmodified",
                              BAD_CAST time);
  }
  if (props & WD_CREATION_DATE) {
    char time[32];
    onion_shortcut_date_string_iso(st.st_mtime, time);
    xmlTextWriterWriteElement(writer, BAD_CAST "lp1:creationdate",
                              BAD_CAST time);
  }
  if (props & WD_CONTENT_LENGTH && !S_ISDIR(st.st_mode)) {
    snprintf(tmp, sizeof(tmp), "%d", (int)st.st_size);
    xmlTextWriterWriteElement(writer, BAD_CAST "lp1:getcontentlength",
                              BAD_CAST tmp);
  }
  if (props & WD_CONTENT_TYPE && S_ISDIR(st.st_mode)) {
    xmlTextWriterWriteElement(writer, BAD_CAST "lp1:getcontenttype",
                              BAD_CAST "httpd/unix-directory");
  }
  if (props & WD_ETAG) {
    char etag[32];
  ...
  if (props & WD_EXECUTABLE && !S_ISDIR(st.st_mode)) {
    if (st.st_mode & 0111)
      xmlTextWriterWriteElement(writer, BAD_CAST "a:executable",
                                BAD_CAST "true");
    else
      xmlTextWriterWriteElement(writer, BAD_CAST "a:executable",
                                BAD_CAST "false");
  ...
  xmlTextWriterEndElement(writer);
  xmlTextWriterWriteElement(writer, BAD_CAST "D:status",
                            BAD_CAST "HTTP/1.1 200 OK");
  ...
  if (props & WD_CONTENT_LENGTH && S_ISDIR(st.st_mode)) {
    snprintf(tmp, sizeof(tmp), "%d", (int)st.st_size);
    xmlTextWriterWriteElement(writer, BAD_CAST "g0:getcontentlength",
                              BAD_CAST "");
  }
  if (props & WD_CONTENT_TYPE && !S_ISDIR(st.st_mode)) {
    xmlTextWriterWriteElement(writer, BAD_CAST "g0:getcontenttype",
                              BAD_CAST "");
  }
  ...
  xmlTextWriterEndElement(writer);
  xmlTextWriterWriteElement(writer, BAD_CAST "D:status",
                            BAD_CAST "HTTP/1.1 404 Not Found");
  xmlTextWriterEndElement(writer);      // /propstat
...
onion_block *onion_webdav_write_propfind(const char *basepath,
  ...
  error =
      onion_webdav_write_props(writer, basepath, realpath, urlpath, NULL,
                               props);
  if (depth > 0) {
    ...
    if (!dir) {
      ...
      while ((de = readdir(dir))) {
        if (de->d_name[0] != '.')
          onion_webdav_write_props(writer, basepath, realpath, urlpath,
                                   de->d_name, props);
      }
      closedir(dir);
    }
...
onion_connection_status onion_webdav_propfind(const char *filename,
  ...
  onion_block *block = onion_webdav_write_propfind(basepath, filename,
                                                   onion_request_get_path(req),
                                                   depth, props);
            ...
            if (strcmp((const char *)set->name, "prop") == 0) {
              ...
              while (prop) {
                ...
                if (strcmp((const char *)prop->name, "executable") == 0) {
                  ...
                  struct stat st;
                  stat(filename, &st);
                  if (prop->children->content[0] == 'T'
                      || prop->children->content[0] == 't') {
...
int onion_webdav_default_check_permissions(const char *exported_path,
                                           ...
                                           onion_request * req) {
  if (strstr(filename, "/../")) {
    ONION_ERROR("Trying to escape! %s is trying to escape from %s", filename,
                exported_path);
    return 1;
  }

  ONION_DEBUG0("Checking permissions for path %s, file %s", exported_path,
               filename);
  char *base, *file;
  int ret = 0;
  ...
  if (!base || !file) {
    ret = 1;
    ONION_ERROR("Could not resolve real path for %s or %s", exported_path,
                filename);
  }
  if ((ret == 0) && strncmp(base, file, strlen(base)) != 0) {
    ret = 1;
    ONION_ERROR("Base %s is not for file %s (%p)", base, file,
                strncmp(base, file, strlen(base)));
  }
...
onion_handler *onion_handler_webdav(const char *path,
  ...
  xmlInitParser();
  LIBXML_TEST_VERSION wd->path = onion_low_strdup(path);
  ...
  onion_handler *ret = onion_handler_new((void *)onion_webdav_handler, wd,
                                         (void *)onion_webdav_free);
  return ret;
}

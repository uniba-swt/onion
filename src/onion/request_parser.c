...
int token_read_STRING(onion_token * token, onion_buffer * data) {
  ...
  while (!is_space(c)) {
    ...
    if (token->pos >= (sizeof(token->str) - 1)) {
      ...
      tmp[15] = '\0';
      ONION_ERROR
          ("Token too long to parse it. Part read start as %s (%d bytes)", tmp,
           token->pos);
...
int token_read_until(onion_token * token, onion_buffer * data, char delimiter) {
  ...
  while (c != delimiter && c != '\n') {
    ...
    if (token->pos >= (sizeof(token->str) - 1)) {
      ONION_ERROR("Token too long to parse it. Part read is %s (%d bytes)",
                  token->str, token->pos);
      return OCS_INTERNAL_ERROR;
    }
    c = data->data[data->pos++];
...
int token_read_KEY(onion_token * token, onion_buffer * data) {
  ...
  if (res == STRING_NEW_LINE) {
    ONION_ERROR
        ("When parsing header, found a non valid delimited string token: '%s'",
         token->str);
    return OCS_INTERNAL_ERROR;
  }
  return res;
...
int token_read_MULTIPART_BOUNDARY(onion_token * token, onion_buffer * data) {
  ...
  while (multipart->boundary[multipart->pos]
         && multipart->boundary[multipart->pos] == data->data[data->pos]) {
    multipart->pos++;
    data->pos++;
    if (data->pos >= data->size)
      return OCS_NEED_MORE_DATA;
  ...
  if (multipart->boundary[multipart->pos]) {
    ONION_ERROR("Expecting multipart boundary, but not here (pos %d) (%c!=%c)",
                multipart->pos, data->data[data->pos],
                multipart->boundary[multipart->pos]);
    return OCS_INTERNAL_ERROR;
  }
...
static onion_connection_status parse_POST_multipart_next(onion_request * req,
                                                         onion_buffer * data);

/**
 * @short Reads from the data to fulfill content-length data.
 */
static onion_connection_status parse_CONTENT_LENGTH(onion_request * req,
                                                    onion_buffer * data) {
  ...
  ONION_DEBUG0
      ("Adding data to request->data (non form POST) %d + %d / %d [%d/%d] %p",
       current_size, length, token->extra_size, skip, data->size,
 ...
 */
static onion_connection_status parse_POST_multipart_file(onion_request * req,
                                                         onion_buffer * data) {
 ...
 * The boundary may be in two or N parts.
 */
static onion_connection_status parse_POST_multipart_data(onion_request * req,
                                                         onion_buffer * data) {
  onion_token *token = req->parser_data;
  onion_multipart_buffer *multipart = (onion_multipart_buffer *) token->extra;
  const char *p = data->data + data->pos;
...
static onion_connection_status parse_POST_multipart_headers_key(onion_request *
                                                                req,
                                                                onion_buffer *
                                                                data);

static onion_connection_status parse_POST_multipart_content_type(onion_request *
                                                                 req,
                                                                 onion_buffer *
                                                                 data) {
  onion_token *token = req->parser_data;
...
static onion_connection_status parse_POST_multipart_ignore_header(onion_request
                                                                  * req,
                                                                  onion_buffer *
                                                                  data) {
  onion_token *token = req->parser_data;
...
static onion_connection_status parse_POST_multipart_start(onion_request * req,
                                                          onion_buffer * data) {
  onion_token *token = req->parser_data;
...
static onion_connection_status parse_POST_urlencode(onion_request * req,
                                                    onion_buffer * data) {
  onion_token *token = req->parser_data;
...
static onion_connection_status parse_headers_KEY(onion_request * req,
                                                 onion_buffer * data);
static onion_connection_status parse_headers_VALUE(onion_request * req,
                                                   onion_buffer * data);

static onion_connection_status
parse_headers_VALUE_skip_leading_whitespace(onion_request * req,
                                            onion_buffer * data) {
...
static onion_connection_status
parse_headers_VALUE_multiline_if_space(onion_request * req,
                                       onion_buffer * data) {
...
static onion_connection_status parse_headers_KEY(onion_request * req,
  ...
  if (res == NEW_LINE) {
    if ((req->flags & OR_METHODS) == OR_POST) {
      ...
      if (content_type
          && (strstr(content_type, "application/x-www-form-urlencoded")
              || strstr(content_type, "boundary")))
...
static onion_connection_status parse_headers_KEY_skip_NL(onion_request * req,
                                                         onion_buffer * data) {
  onion_token *token = req->parser_data;
...
static onion_connection_status parse_headers_VERSION(onion_request * req,
                                                     onion_buffer * data) {
  onion_token *token = req->parser_data;
...
static onion_connection_status parse_headers_URL(onion_request * req,
                                                 onion_buffer * data) {
  onion_token *token = req->parser_data;
  int res = token_read_STRING(token, data);
...
static onion_connection_status parse_headers_GET(onion_request * req,
                                                 onion_buffer * data) {
  onion_token *token = req->parser_data;
  int res = token_read_STRING(token, data);
...
static onion_connection_status prepare_POST(onion_request * req) {
  ...
  if (!content_type
    ...
    if (cl > req->connection.listen_point->server->max_post_size) {
      ONION_ERROR("Asked to send much POST data. Limit %d. Failing.",
  ...
  onion_multipart_buffer *multipart =
      onion_low_malloc(token->extra_size + sizeof(onion_multipart_buffer) +
                       mp_token_size + 2);
  assert(token->extra == NULL);
  ...
  multipart->data =
      (char *)multipart + sizeof(onion_multipart_buffer) + multipart->size + 1;
...
static onion_connection_status prepare_CONTENT_LENGTH(onion_request * req) {
  ...
  if (cl > req->connection.listen_point->server->max_post_size) {
    ONION_ERROR("Trying to set more data at server than allowed %d",
                req->connection.listen_point->server->max_post_size);
    return OCS_INTERNAL_ERROR;
  }

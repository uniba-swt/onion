...
onion *onion_new(int flags) {
  ONION_DEBUG0
      ("Some internal sizes: onion size: %d, request size %d, response size %d",
       sizeof(onion), sizeof(onion_request), sizeof(onion_response));
  if (SOCK_CLOEXEC == 0) {
    ONION_WARNING
        ("There is no support for SOCK_CLOEXEC compiled in libonion. This may be a SECURITY PROBLEM as connections may leak into executed programs.");
  }
  ...
  o->internal_error_handler =
      onion_handler_new((onion_handler_handler) onion_default_error, NULL,
                        NULL);
...
void
onion_set_client_data(onion * server, void *data,
...
static void shutdown_server(int _) {
  if (shutdown_server_first_call) {
    if (last_onion) {
      ...
      ONION_INFO("Exiting onion listening (SIG%s)",
                 _ == SIGTERM ? "TERM" : "INT");
    }
  } else {
...
int onion_listen(onion * o) {
  ...
  if (!(o->flags & O_DETACHED) && (o->flags & O_DETACH_LISTEN)) {       // Must detach and return
    o->flags |= O_DETACHED;
    int errcode =
        onion_low_pthread_create(&o->listen_thread, NULL, onion_listen_start,
                                 o);
    if (errcode != 0)
      return errcode;
    return 0;
  ...
  if (o->flags & O_ONE) {
    ...
    if (listen_points[1] != NULL) {
      ONION_WARNING
          ("Trying to use non-poll and non-thread mode with several listen points. Only the first will be listened");
    }
    ...
    while (*listen_points) {
      ...
      onion_poller_slot *slot =
          onion_poller_slot_new(p->listenfd, (void *)onion_listen_point_accept,
                                p);
...
#ifdef HAVE_PTHREADS
    ONION_DEBUG("Start polling / listening %p, %p, %p", o->listen_points,
                *o->listen_points, *(o->listen_points + 1));
    if (o->flags & O_THREADED) {
      o->threads = onion_low_malloc(sizeof(pthread_t) * (o->nthreads - 1));
      int i;
      for (i = 0; i < o->nthreads - 1; i++) {
        onion_low_pthread_create(&o->threads[i], NULL, onion_poller_poll_start,
                                 o->poller);
      }
 ...
 */
int onion_add_listen_point(onion * server, const char *hostname,
                           const char *port, onion_listen_point * protocol) {
  if (protocol == NULL) {
  ...
  ONION_DEBUG("add %p listen_point (%p, %p, %p)", protocol,
              server->listen_points, *server->listen_points,
              *(server->listen_points + 1));
  return 0;
}
...
onion_url *onion_root_url(onion * server) {
  if (server->root_handler) {
    ...
    ONION_WARNING
        ("Could not get root url handler, as there is another non url handler at root.");
    return NULL;
  }
...
int onion_set_certificate(onion * onion, onion_ssl_certificate_type type,
                          const char *filename, ...) {
  va_list va;
...
/// Set a certificate for use in the connection
int onion_set_certificate_va(onion * onion, onion_ssl_certificate_type type,
  ...
  if (!onion->listen_points) {
    ...
    if (first_listen_point->write != onion_https_write) {
      if (first_listen_point->write != onion_http_write) {
        ONION_ERROR
            ("First listen point is not HTTP not HTTPS. Refusing to promote it to HTTPS. Use proper onion_https_new.");
        return -1;
      }
  ...
  int r =
      onion_https_set_certificate_argv(onion->listen_points[0], type, filename,
                                       va);
